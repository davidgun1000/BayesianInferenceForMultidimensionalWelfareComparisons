...
function [w_beta,m_beta,s_beta,scale_beta_com,accept_w_beta,accept_beta_com,health_tilde] = sampling_beta_parameter(data,...
    w_beta,m_beta,s_beta,theta_gauss,prior,iter,scale_beta_com,V1_beta_com,...
    accept_w_beta,accept_beta_com,num_component,target_accept,income_tilde,educ_tilde,happiness_tilde,dim_cop)
...
for k=1:num_component
    dens1=dens1+w_beta(k)*betaden(health_data,s_beta(k),m_beta(k));
    cdf1=cdf1+w_beta(k)*betacdf(health_data,s_beta(k)*m_beta(k),s_beta(k)*(1-m_beta(k)));
end
lik_weight=sum(log(dens1));
health_tilde=norminv(cdf1);
prior_weight = logdir3pdf_general(w_beta',[prior.phi*ones(1,num_component)]);
...
w_beta_star = dirich_rnd(prior.c1.*w_beta')';
if sum((w_beta_star>0.999)>0)
else
cdf1_star = 0;
dens1_star = 0;
for k=1:num_component
    dens1_star=dens1_star+w_beta_star(k)*betaden(health_data,s_beta(k),m_beta(k));
    cdf1_star=cdf1_star+w_beta_star(k)*betacdf(health_data,s_beta(k)*m_beta(k),s_beta(k)*(1-m_beta(k)));
end
lik_weight_star = sum(log(dens1_star));
health_tilde_star = norminv(cdf1_star);
prior_weight_star = logdir3pdf_general(w_beta_star',[prior.phi*ones(1,num_component)]);
post_weight_star = prior_weight_star + lik_weight_star + sum(log(mvnpdf([income_tilde,health_tilde_star,educ_tilde,happiness_tilde],zeros(1,dim_cop),covmat))) - ... 
                  sum(log(normpdf(income_tilde,0,1))+log(normpdf(health_tilde_star,0,1))+log(normpdf(educ_tilde,0,1))+log(normpdf(happiness_tilde,0,1)));
logwstar_w = logdir3pdf_general(w_beta',[prior.c1.*w_beta_star']);
%logwstar_w_alt = log(dir3pdf(w_gamma',prior.c1*w_gamma_star(1),prior.c1*w_gamma_star(2)));
logw_wstar = logdir3pdf_general(w_beta_star',[prior.c1.*w_beta']);
r1 = exp(post_weight_star - post_weight + logwstar_w - logw_wstar);
C1 = min(1,r1);
if A1<=C1
   w_beta=w_beta_star; 
   accept_w_beta=accept_w_beta+1; 
end
 end
for k=1:num_component
    A1=rand();
    ...
    for j=1:num_component
        dens1=dens1+w_beta(j)*betaden(health_data,s_beta(j),m_beta(j));
        cdf1=cdf1+w_beta(j)*betacdf(health_data,s_beta(j)*m_beta(j),s_beta(j)*(1-m_beta(j)));
    end
    lik_com=sum(log(dens1));
    health_tilde=norminv(cdf1);
    prior_com = log(betapdf(m_beta(k),prior.alpha_beta,prior.beta_beta))+log(exppdf(s_beta(k),1/prior.theta_beta));
    post_com = prior_com+lik_com+...     
               sum(log(mvnpdf([income_tilde,health_tilde,educ_tilde,happiness_tilde],zeros(1,dim_cop),covmat))) - ... 
               sum(log(normpdf(income_tilde,0,1))+log(normpdf(health_tilde,0,1))+log(normpdf(educ_tilde,0,1))+log(normpdf(happiness_tilde,0,1))); 
    jac_com = log(1/s_beta(k))+log((1/m_beta(k))+(1/(1-m_beta(k))));
    theta_com = [log(s_beta(k)),logit_inverse(m_beta(k))];
    R1_com = mvnrnd(theta_com,scale_beta_com(k).*V1_beta_com(:,:,k));
    s_beta_star = s_beta;
    s_beta_star(k) = exp(R1_com(1,1));
    m_beta_star = m_beta;
    m_beta_star(k) = logit_cdf_com(R1_com(1,2));
    if m_beta_star(k)>0.99
    C1_com(k) = 0;
    else
    dens1_star=0;
    cdf1_star=0;
    for j=1:num_component
        dens1_star=dens1_star+w_beta(j)*betaden(health_data,s_beta_star(j),m_beta_star(j));
        cdf1_star=cdf1_star+w_beta(j)*betacdf(health_data,s_beta_star(j)*m_beta_star(j),s_beta_star(j)*(1-m_beta_star(j)));
    end
    lik_com_star = sum(log(dens1_star));
    health_tilde_star = norminv(cdf1_star);
    prior_com_star = log(betapdf(m_beta_star(k),prior.alpha_beta,prior.beta_beta))+log(exppdf(s_beta_star(k),1/prior.theta_beta));
    post_com_star = prior_com_star+lik_com_star+...     
                    sum(log(mvnpdf([income_tilde,health_tilde_star,educ_tilde,happiness_tilde],zeros(1,dim_cop),covmat))) - ... 
                    sum(log(normpdf(income_tilde,0,1))+log(normpdf(health_tilde_star,0,1))+log(normpdf(educ_tilde,0,1))+log(normpdf(happiness_tilde,0,1)));  
    jac_com_star = log(1/s_beta_star(k))+log((1/m_beta_star(k))+(1/(1-m_beta_star(k))));
    r1 = exp(post_com_star - post_com +jac_com -jac_com_star);
    C1_com(k) = min(1,r1);
    if A1<=C1_com(k)
       m_beta(k) = m_beta_star(k);
       s_beta(k) = s_beta_star(k);
       accept_beta_com(k) = accept_beta_com(k)+1;
...
if iter>1000
    for k=1:num_component
        scale_beta_com(k) = update_sigma(scale_beta_com(k),C1_com(k),target_accept,iter,2);
        if scale_beta_com(k)>1
             scale_beta_com(k)=1; 
        end
    end
end

%col = [m_beta',s_beta',w_beta'];
