...
function [w_beta,m_beta,s_beta,scale_beta_com,accept_w_beta,accept_beta_com] = sampling_beta_parameter_only_Mix(data,...
    w_beta,m_beta,s_beta,prior,iter,scale_beta_com,V1_beta_com,...
    accept_w_beta,accept_beta_com,num_component,target_accept)
...
post_weight_star = prior_weight_star + lik_weight_star;
...
for k=1:num_component
    ...
    %prior_com = log(1-exp(-prior.del*((s_beta(k)-2)^2+(m_beta(k)-0.5)^2)))+((-prior.rho/(s_beta(k)^2*m_beta(k)*(1-m_beta(k))))-((prior.kap*s_beta(k)^2)/2));
    ...
    %prior_com_star = log(1-exp(-prior.del*((s_beta_star(k)-2)^2+(m_beta_star(k)-0.5)^2)))+((-prior.rho/(s_beta_star(k)^2*m_beta_star(k)*(1-m_beta_star(k))))-((prior.kap*s_beta_star(k)^2)/2));    
