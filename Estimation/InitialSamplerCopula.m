...
% rng(seednum);
function [prior,Post,V1_gamma_com,V1_beta_com,V1_delta_educ,V1_delta_happiness,scale_theta_copula]=InitialSamplerCopula(data,num_component_gamma,num_component_beta)
%load('simulated_data_x.mat');
num_category_educ = length(unique(data(:,3)));
num_category_happiness = length(unique(data(:,4)));
dim_cop=4;
prior.phi=1;
prior.theta_gamma=0.01;
prior.alpha_gamma=10;
prior.beta_gamma=[20,50,100,200];
prior.theta_beta=0.1;
prior.alpha_beta=5;
prior.beta_beta=5;
prior.delta_var_educ = 100*eye(num_category_educ-1);
prior.delta_var_happiness = 100*eye(num_category_happiness-1);
prior.c1=1000;
burn = 10000;
nit = 20000;
s=burn+nit;
...
for k=1:num_category_educ
    prop_init_educ(1,k) = mean(data(:,3)==k);
end

for k=1:num_category_happiness
    prop_init_happiness(1,k) = mean(data(:,4)==k);
end

cum_marginal_educ = cumsum(prop_init_educ(1:num_category_educ-1));
cum_marginal_happiness = cumsum(prop_init_happiness(1:num_category_happiness-1));
limit_educ = norminv(cum_marginal_educ);
limit_educ(1,num_category_educ) = norminv(0.9999);
limit_happiness = norminv(cum_marginal_happiness);
limit_happiness(1,num_category_happiness) = norminv(0.9999);
for j=1:num_category_educ-1
    if j==1
       delta_educ(1,j)=log(limit_educ(j)+4);
    else
       delta_educ(1,j)=log(limit_educ(j)-limit_educ(j-1));
    end    
end
%for k=1:num_category_educ
%    prop_educ(1,k)=mean(data(:,3)==k);
%end
for j=1:num_category_happiness-1
    if j==1
       delta_happiness(1,j)=log(limit_happiness(j)+4);
    else
       delta_happiness(1,j)=log(limit_happiness(j)-limit_happiness(j-1));
    end    
end
%for k=1:num_category_happiness
%    prop_happiness(1,k)=mean(data(:,4)==k); 
%end
...
w_beta = ones(1,num_component_beta)./num_component_beta;
sort_health = sort(data(:,2));
for k=1:num_component_beta
    if k==1
       X = sort_health(1:round((k/num_component_beta)*length(sort_health(:,1))));         
    else
       X = sort_health(round(((k-1)/num_component_beta)*length(sort_health(:,1))):round(((k)/num_component_beta)*length(sort_health(:,1))));  
    end
    phat(k,:) = mle(X,'distribution','beta');
end

s_beta = (phat(:,1) + phat(:,2))';
m_beta = (phat(:,1)./(phat(:,1) + phat(:,2)))';
%m_beta = 
%m_beta = [0.6,0.75,0.9];
%s_beta = 5*ones(1,num_component_beta);
%s_beta = [1, 7, 22];
theta_gauss=zeros(dim_cop*(dim_cop-1)/2,1);

for j=1:num_component_gamma
    V1_gamma_com(:,:,j)=0.001*eye(2);
    scale_gamma_com(1,j)=0.1;
end

for j=1:num_component_beta
    V1_beta_com(:,:,j)=0.001*eye(2);
    scale_beta_com(1,j)=0.1;
end

V1_delta_educ = 0.001*eye(num_category_educ-1);
scale_delta_educ = 0.01;

V1_delta_happiness = 0.001*eye(num_category_happiness-1);
scale_delta_happiness = 0.01;
accept_w_gamma=0;
accept_gamma_com=zeros(num_component_gamma,1);
accept_w_beta=0;
accept_beta_com=zeros(num_component_beta,1);
accept_delta_educ=0;
accept_delta_happiness=0;
target_accept=0.2;
...
cdf1 = zeros(length(data(:,1)),1);
dens1 = zeros(length(data(:,1)),1);
for k=1:num_component_beta
    dens1=dens1+w_beta(k)*betaden(data(:,2),s_beta(k),m_beta(k));
    cdf1=cdf1+w_beta(k)*betacdf(data(:,2),s_beta(k)*m_beta(k),s_beta(k)*(1-m_beta(k)));
end
health_tilde=norminv(cdf1);
...
educ_tilde = zeros(length(data(:,1)),1);
for k=1:num_category_educ
    if k==1
       educ_tilde(id(:,k),1) = trandn(-inf*ones(sum(id(:,k)),1),(limit_educ(1,k))*ones(sum(id(:,k)),1));
    
    else
       educ_tilde(id(:,k),1) = trandn((limit_educ(1,k-1))*ones(sum(id(:,k)),1),(limit_educ(1,k))*ones(sum(id(:,k)),1)); 
    end
end

happiness_tilde = zeros(length(data(:,1)),1);

for k=1:num_category_happiness
    id(:,k) = (data(:,dim_happiness)==k);    
end

for k=1:num_category_happiness
    if k==1
       happiness_tilde(id(:,k),1) = trandn(-inf*ones(sum(id(:,k)),1),(limit_happiness(1,k))*ones(sum(id(:,k)),1));
    
    else
       happiness_tilde(id(:,k),1) = trandn((limit_happiness(1,k-1))*ones(sum(id(:,k)),1),(limit_happiness(1,k))*ones(sum(id(:,k)),1)); 
    end
end
...
scale_theta_copula=0.01*ones(1,length(theta_gauss));
accept_theta_copula=zeros(1,length(theta_gauss));

for i=1:s
    i
    m_beta'
    %w_gamma'
    [w_gamma,m_gamma,v_gamma,scale_gamma_com,accept_w_gamma,accept_gamma_com,income_tilde]=sampling_gamma_parameter(data,w_gamma,m_gamma,v_gamma,theta_gauss,prior,i,scale_gamma_com,V1_gamma_com,...
                                                                                                       accept_w_gamma,accept_gamma_com,num_component_gamma,target_accept,health_tilde,educ_tilde,happiness_tilde,dim_cop);
    ...
    if i>1000
        for k=1:num_component_gamma
            V1_gamma_com(:,:,k) = cov(thetasave_gamma{k,1}(i-1000:i,:));
            V1_gamma_com(:,:,k) = jitChol(V1_gamma_com(:,:,k));
        end
    end
    
    if i>burn
    temp.w_gamma(i-burn,:) = w_gamma;
    temp.m_gamma(i-burn,:) = m_gamma;
    temp.v_gamma(i-burn,:) = v_gamma;
    end
    [w_beta,m_beta,s_beta,scale_beta_com,accept_w_beta,accept_beta_com,health_tilde]=sampling_beta_parameter(data,w_beta,m_beta,s_beta,theta_gauss,prior,i,scale_beta_com,V1_beta_com,...
                                                                       accept_w_beta,accept_beta_com,num_component_beta,target_accept,income_tilde,educ_tilde,happiness_tilde,dim_cop);
                                                                   
    for k=1:num_component_beta
        thetasave_beta{k,1}(i,:) = [log(s_beta(k)),logit_inverse(m_beta(k))];
    end
    
    if i>1000
        for k=1:num_component_beta
            V1_beta_com(:,:,k) = cov(thetasave_beta{k,1}(i-1000:i,:));
            V1_beta_com(:,:,k) = jitChol(V1_beta_com(:,:,k));
        end
    end
    
    if i>burn
    temp.w_beta(i-burn,:) = w_beta;
    temp.m_beta(i-burn,:) = m_beta;
    temp.s_beta(i-burn,:) = s_beta;                                                               
    end 
    [delta_educ,scale_delta_educ,accept_delta_educ,educ_tilde]=sample_categorical_parameter(data,delta_educ,theta_gauss,prior,i,scale_delta_educ,V1_delta_educ,...
                                                                        accept_delta_educ,target_accept,income_tilde,health_tilde,happiness_tilde,dim_cop,dim_educ);
    
    
    for k=1:num_category_educ-1
       if k==1
    ...
    limit_educ(1,num_category_educ) = norminv(0.9999);
    cum_marginal_educ = normcdf(limit_educ);
    
    for k=1:num_category_educ
       if k==1 
       prop_educ(1,k)=cum_marginal_educ(1,k);
       else
       prop_educ(1,k)=cum_marginal_educ(1,k)-cum_marginal_educ(1,k-1);    
       end
    end
                                                                    
    thetasave_delta_educ(i,:) = delta_educ;                                                           
    if i>1000
       V1_delta_educ = cov(thetasave_delta_educ(i-1000:i,:));
       V1_delta_educ = jitChol(V1_delta_educ);
        
    end
    if i>burn
    temp.delta_educ(i-burn,:) = delta_educ;
    temp.prop_educ(i-burn,:) = prop_educ;
    end
    [delta_happiness,scale_delta_happiness,accept_delta_happiness,happiness_tilde]=sample_categorical_parameter(data,delta_happiness,theta_gauss,prior,i,scale_delta_happiness,V1_delta_happiness,...
                                                                        accept_delta_happiness,target_accept,income_tilde,health_tilde,educ_tilde,dim_cop,dim_happiness);
                                                                    
    for k=1:num_category_happiness-1
       if k==1
          limit_happiness(1,k)= -4 + exp(delta_happiness(1,k));
       else
          limit_happiness(1,k)= limit_happiness(1,k-1) + exp(delta_happiness(1,k)); 
       end    
    end
    limit_happiness(1,num_category_happiness) = norminv(0.9999);
    cum_marginal_happiness = normcdf(limit_happiness);
    
    for k=1:num_category_happiness
       if k==1 
       prop_happiness(1,k)=cum_marginal_happiness(1,k);
       else
       prop_happiness(1,k)=cum_marginal_happiness(1,k)-cum_marginal_happiness(1,k-1);    
       end
    end
                                                                    
    thetasave_delta_happiness(i,:) = delta_happiness;                                                           
    if i>1000
       V1_delta_happiness = cov(thetasave_delta_happiness(i-1000:i,:));
       V1_delta_happiness = jitChol(V1_delta_happiness);
        
    end
    if i>burn
    temp.delta_happiness(i-burn,:) = delta_happiness;
    temp.prop_happiness(i-burn,:) = prop_happiness;
    end             
    [theta_gauss,scale_theta_copula,accept_theta_copula]=sampling_copula_parameter(data,theta_gauss,prior,i,scale_theta_copula,...
                                                                        accept_theta_copula,target_accept,income_tilde,health_tilde,educ_tilde,happiness_tilde,dim_cop);
    if i>burn
    temp.theta_gauss(i-burn,:) = theta_gauss;
    end                                                       
                                                                    
end

Post.w_gamma = mean(temp.w_gamma);
Post.m_gamma = mean(temp.m_gamma);
Post.v_gamma = mean(temp.v_gamma);

Post.w_beta = mean(temp.w_beta);
Post.m_beta = mean(temp.m_beta);
Post.s_beta = mean(temp.s_beta);

Post.delta_educ = mean(temp.delta_educ);
Post.prop_educ = mean(temp.prop_educ);

Post.delta_happiness = mean(temp.delta_happiness);
Post.prop_happiness = mean(temp.prop_happiness);

Post.theta_gauss = mean(temp.theta_gauss);

Post.income_tilde = income_tilde;
Post.health_tilde = health_tilde;
Post.educ_tilde = educ_tilde;
Post.happiness_tilde = happiness_tilde; 
