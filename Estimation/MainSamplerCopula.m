%MCMC sampling scheme for sampling all copula model parameters

function [w_gamma_temp,m_gamma_temp,v_gamma_temp,...
          w_beta_temp,m_beta_temp,s_beta_temp,...
          delta_educ_temp,prop_educ_temp,delta_happiness_temp,...
          prop_happiness_temp,theta_gauss_temp]=MainSamplerCopula(data,prior,init_param,V1_gamma_com,V1_beta_com,V1_delta_educ,V1_delta_happiness,scale_theta_copula,num_component_gamma,num_component_beta)
num_category_educ = length(unique(data(:,3))); % number of categories in the education variable
num_category_happiness = length(unique(data(:,4))); % number of categories in the happiness variable
dim_cop=4; % the dimension of well-being attributes

burn = 1000; % burn in
nit = 5000; % sampling 
s=burn+nit; %total number of MCMC iterations

%initial values for all parameters

prop_init_educ = init_param.prop_educ;
prop_init_happiness = init_param.prop_happiness; 
...
w_gamma = init_param.w_gamma;
m_gamma = init_param.m_gamma;
v_gamma = init_param.v_gamma;

w_beta = init_param.w_beta;
m_beta = init_param.m_beta;
s_beta = init_param.s_beta;

theta_gauss=init_param.theta_gauss';
...
scale_delta_educ = 0.1;

scale_delta_happiness = 0.1;
accept_w_gamma=0;
...
target_accept=0.2;
income_tilde = init_param.income_tilde;
health_tilde = init_param.health_tilde;
educ_tilde = init_param.educ_tilde;
happiness_tilde = init_param.happiness_tilde;
...
for i=1:s
    ...
    if i>1000
        for k=1:num_component_beta
            V1_beta_com(:,:,k) = cov(thetasave_beta{k,1}(1:i,:));
    ...
    if i>1000
       V1_delta_educ = cov(thetasave_delta_educ(1:i,:));
    ...
    if i>1000
       V1_delta_happiness = cov(thetasave_delta_happiness(1:i,:));
    ...
    if i>burn & mod(i,100)==0
        ...
        w_gamma_temp(ii,:) = w_gamma;
        m_gamma_temp(ii,:) = m_gamma;
        v_gamma_temp(ii,:) = v_gamma;
        
        w_beta_temp(ii,:) = w_beta;
        m_beta_temp(ii,:) = m_beta;
        s_beta_temp(ii,:) = s_beta; 
        
        delta_educ_temp(ii,:) = delta_educ;
        prop_educ_temp(ii,:) = prop_educ;
        
        delta_happiness_temp(ii,:) = delta_happiness;
        prop_happiness_temp(ii,:) = prop_happiness;
        
        theta_gauss_temp(ii,:) = theta_gauss;
        ii=ii+1;
    end
